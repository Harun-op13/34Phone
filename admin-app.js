...
const app = {
                        ...
                        <tr class="${p.sold ? 'sold-row' : ''}">
                            <td>
                                <div style="display: flex; align-items: center; gap: 0.5rem;">
                                    ${p.media && p.media[0] ? `<img src="${p.media[0].url}" style="width: 30px; height: 30px; border-radius: 4px; object-fit: cover;">` : ''}
                                    <div style="${p.sold ? 'opacity: 0.6;' : ''}">
                                        <div style="font-weight: 600;">${p.name}</div>
                                        <div style="font-size: 0.7rem; color: var(--text-muted)">${p.brand} | ${p.storage} | ${p.ram ? p.ram + ' | ' : ''} ${p.color}</div>
                                    </div>
                                </div>
                            </td>
                             <td>${p.price} دج</td>
    ...
    handlers: {
        openProdModal() {
            ...
            document.getElementById('prod-ram').value = '';
        ...
        async saveProduct() {
            try {
                ...
                const ram = document.getElementById('prod-ram').value;
